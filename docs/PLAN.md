...
| 图身后是什么颜色 | 不知道就不能走「烘焙底色」这条 8~24x 的路 | `HostBackgroundResolver` 解析布局父链；安全时回退保留 alpha |
...
下拉框选项顺序（默认选第一个）：
解析成功时：`宿主底色（解析得到）` → `白` → `灰` → `黑` → `棋盘格`；
解析不到时默认 `棋盘格`，再列白/灰/黑；也可输入 `#RGB` / `#RRGGBB` 自定义预览色。
自定义色只影响预览，不参与压缩决策。只有压缩路线依赖底色时，info 栏才提示「预览仅供参考」。
...
宿主底色未解析出来时不选择 `BAKE_BACKGROUND`；保留 alpha 仍有收益则自动回退，否则跳过并计入报告。
...
- 压缩后引入色带（这正是最想避免的失真）
- 宿主底色没解析出来，且保留 alpha 的安全路线也收益不足（猜白色会在线上留下色差方块）
...
2. **批量应用要守住和单文件一样的门禁**。底色未确定时只能走保留 alpha 的安全回退；
   该路线收益不足则跳过，不能因为是批量就猜白色 —— 否则会在线上留下色差方块。
...
| 常量 | 值 | 依据 |
|---|---|---|
| `HARD_ALPHA_SEMI_RATIO` | 0.02 | 半透明>2% 即渐变型，优先烘焙；底色未知但 KEEP_ALPHA 达到收益门限时安全回退 |
